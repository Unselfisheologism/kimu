...
import { type MediaBinItem, type ScrubberState, type TimelineState, type TrackState, type TimelineDataItem, type RemotionCodeData, FPS } from "~/components/timeline/types";
...
// ============================
// REMOTION CODE OPERATIONS
// ============================

export function llmWriteRemotionCode(
  description: string,
  ...
  // This function creates the Remotion code data structure
  // The actual code will be generated by the AI based on the description
  return {
    code: "", // Will be populated by AI
    compositionName,
    durationInFrames,
    width,
    height,
...
export function llmCreateRemotionCodeScrubber(
  codeData: RemotionCodeData,
  trackNumber: number,
  positionSeconds: number,
  pixelsPerSecond: number,
  timeline: TimelineState,
  ...
  // Create the media bin item for the Remotion code
  const remotionItem: MediaBinItem = {
    id: generateUUID(),
    mediaType: "remotion_code",
    mediaUrlLocal: null,
    mediaUrlRemote: null,
    media_width: codeData.width,
    media_height: codeData.height,
    text: null,
    groupped_scrubbers: null,
    left_transition_id: null,
    right_transition_id: null,
    name: codeData.compositionName,
    durationInSeconds: codeData.durationInFrames / codeData.fps,
    uploadProgress: null,
    isUploading: false,
    remotionCode: codeData,
  };

  handleDropOnTrack(remotionItem, trackId, dropLeftPx);
  return remotionItem;
}

export function llmAddRemotionCodeToMediaBin(
  codeData: RemotionCodeData,
  setMediaBinItems: (items: MediaBinItem[]) => void,
  mediaBinItems: MediaBinItem[]
): MediaBinItem {
  ...
  setMediaBinItems([...mediaBinItems, remotionItem]);
  return remotionItem;
}

export function llmGetRemotionCodeFromMediaBin(
  itemId: string,
  mediaBinItems: MediaBinItem[]
): RemotionCodeData | null {
  const item = mediaBinItems.find(i => i.id === itemId);
  if (item && item.mediaType === "remotion_code" && item.remotionCode) {
    return item.remotionCode;
  }
  return null;
}
